...
\node[method, below=0.5cm of phase2] (execute) {\texttt{ExecuteUnified()}\\or\\ExecuteUnifiedByAddress()};
...
\item \textbf{Attributes}: Custom transaction attribute containing:
  ...
  \item Nonce (account-scoped replay counter)
...
The master contract's canonical \code{executeUnifiedByAddress} method (with legacy meta wrappers preserved for compatibility):
\begin{enumerate}[leftmargin=*,topsep=2pt,itemsep=1pt]
...
\item Called by \code{ExecuteUnified/ExecuteUnifiedByAddress} during Application phase (pure Neo execution path)
...
\node[cell, fill=purple!10] (c32) at (4,-4) {executeUnified(\\accountAddress,\\evmPubKeys[],\\evmSigs[])};
\node[cell, fill=purple!10] (c33) at (8,-4) {executeUnified(\\accountAddress,\\evmPubKeys[],\\evmSigs[])};
...
\node[cell, fill=cyan!10] (c52) at (4,-7) {executeUnified()\\CheckMixed\\(EVM only)};
\node[cell, fill=cyan!10] (c53) at (8,-7) {executeUnified()\\CheckMixed\\(N3 + EVM)};
...
\item Transaction calls \code{ExecuteUnifiedByAddress}
...
\item Transaction includes Neo witness and calls \code{ExecuteUnifiedByAddress}
\item Master contract verifies EIP-712 signature:
  \begin{itemize}[leftmargin=*,topsep=1pt,itemsep=0pt]
