...
\node[check, below=0.3cm of verify] (checkn3) {CheckNative\\Signatures\\(N3 only)};
...
\node[method, below=0.5cm of phase2] (execute) {\texttt{Execute()}\\or\\ executeMetaTx\\ByAddress()};
\node[check, below=0.3cm of execute] (checkmixed) {CheckMixed\\Signatures\\(N3 + EVM)};
...
\caption{Two-phase verification in Neo-AA: the verification trigger authenticates native Neo paths; the application trigger combines Neo and EVM signers.}
...
% Start
\node[start] (start) at (0,0) {CheckMixed\\Signatures(\\roles, thresh,\\signers)};
...
% Check EVM signature
\node[decision, below=1.2cm of loop] (checkevm) {roles[i] in\\verifiedSig\\ners[]?};
...
\caption{Unified threshold evaluation in CheckMixedSignatures: EVM signers and Neo witnesses counted against one policy threshold.}
...
\item Master contract verifies EIP-712 signature:
  \begin{itemize}[leftmargin=*,topsep=1pt,itemsep=0pt]
  \item Reconstruct typed hash:
        $h = \text{keccak256}(\texttt{0x1901} \| D_s \| M_s)$
  \item Verify: $\text{VerifyECDsa}(h, pk, \sigma, \texttt{secp256k1})$
  \item Recover: $a_{\text{rec}} = \text{DeriveEthAddr}(pk)$
  \end{itemize}
...
\end{tikzpicture}
\caption{Mixed authorization workflow: EVM typed-data signatures combined with Neo witness material, evaluated under one threshold.}
