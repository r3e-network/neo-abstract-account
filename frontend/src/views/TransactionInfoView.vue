<template>
  <div class="relative min-h-screen bg-ata-dark overflow-hidden font-sans text-slate-300">
    ...
    <div class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fade-in-up">
      <div class="dark-panel-override glass-panel p-6 sm:p-10 rounded-lg relative">
        <template v-if="draftId">
          ...
          <p class="text-sm text-slate-400 mb-8 max-w-2xl">{{ t('sharedDraft.subtitle', 'Load an immutable transaction draft, review collected approvals, append a new signature, and choose the final client-side or relay broadcast path.') }}</p>
          ...
          <div class="grid gap-4 mb-8 lg:grid-cols-4">
            <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Draft ID</p>
              <code class="block text-sm font-mono text-slate-300 break-all">{{ draftId }}</code>
            </div>
            <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Share URL</p>
              <code class="block text-sm font-mono text-slate-300 break-all">{{ shareUrl || 'Loading…' }}</code>
            </div>
            <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">{{ t('sharedDraft.collaboratorLinkLabel', 'Collaborator Link') }}</p>
              <code class="block text-sm font-mono text-slate-300 break-all">{{ collaborationUrl || 'Read-only access' }}</code>
            </div>
            <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">{{ t('sharedDraft.operatorLinkLabel', 'Operator Link') }}</p>
              <code class="block text-sm font-mono text-slate-300 break-all">{{ operatorUrl || 'Operator access required' }}</code>
            </div>
            <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Status</p>
              <div class="text-sm font-medium text-slate-300">{{ draft?.status || 'loading' }}</div>
            </div>
          </div>
          <div v-if="draft && accessScope === 'read'" class="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-400 font-medium">
          ...
          <div v-else-if="draft && accessScope === 'sign'" class="mb-6 rounded-lg border border-ata-blue/30 bg-ata-blue/10 px-4 py-3 text-sm text-ata-blue font-medium">
          ...
          <div v-if="loading" class="text-sm text-slate-400 font-medium">Loading shared draft…</div>
          ...
          <div v-else-if="draft" class="space-y-6">
            <div class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              <section class="rounded-lg border border-ata-border bg-ata-panel p-5 shadow-sm">
                <div class="mb-4">
                  <h2 class="text-base font-bold text-white uppercase tracking-widest font-mono">Operation Snapshot</h2>
                  <p class="text-sm text-slate-400">Key execution details, relay state, and payload availability for this immutable draft.</p>
                </div>
                <div class="grid gap-3 md:grid-cols-2">
                  <div v-for="item in operationSnapshotItems" :key="item.label" class="rounded-lg border border-ata-border bg-ata-panel p-4">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">{{ item.label }}</p>
                    <div class="mt-1 text-sm font-medium text-slate-300 break-all">{{ item.value }}</div>
              ...
              <section class="rounded-lg border border-ata-border bg-ata-panel p-5 shadow-sm">
                <div class="mb-4">
                  <h2 class="text-base font-bold text-white uppercase tracking-widest font-mono">Signer Checklist</h2>
                  <p class="text-sm text-slate-400">{{ signerProgress.signatureCount }}/{{ signerProgress.requiredCount }} required approvals collected{{ signerProgress.pending.length ? ` · ${signerProgress.pending.length} still pending` : signerProgress.requiredCount ? ' · all required signers satisfied' : ' · no required signer roster recorded' }}.</p>
                </div>
                <div v-if="signerChecklistItems.length === 0" class="rounded-lg border border-dashed border-ata-border bg-ata-panel p-4 text-sm text-slate-400">No signer checklist has been recorded for this draft yet.</div>
                <div v-else class="space-y-3">
                  <div v-for="item in signerChecklistItems" :key="item.key" class="flex items-start justify-between gap-3 rounded-lg border border-ata-border bg-ata-panel p-4">
                    <div class="min-w-0">
                      <div class="text-sm font-semibold text-slate-300 break-all">{{ item.label }}</div>
                      <div class="mt-1 text-xs text-slate-400">{{ item.detail }}</div>
                      <code v-if="item.signaturePreview" class="mt-2 block text-xs text-slate-400">{{ item.signaturePreview }}</code>
                    </div>
                    <span :class="item.status === 'Collected' ? 'bg-ata-green/10 text-ata-green border-ata-green/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30'" class="shrink-0 rounded-full border px-2 py-0.5 text-xs font-semibold uppercase tracking-wide">{{ item.status }}</span>
                  </div>
                </div>
              </section>
            </div>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
              <section class="rounded-lg border border-ata-border bg-ata-panel p-5 shadow-sm">
                <div class="mb-4">
                  <h2 class="text-base font-bold text-white uppercase tracking-widest font-mono">Signature Actions</h2>
                  <p class="text-sm text-slate-400">Connect wallets, paste external approvals, or append a contract-aligned EVM signature to the shared draft.</p>
                </div>
                <div class="mb-4 flex flex-wrap gap-2">
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel" @click="connectNeoWallet">{{ t('operations.connectNeoWallet', 'Connect Neo Wallet') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel" @click="connectEvmWallet">{{ t('operations.connectEvmWallet', 'Connect EVM Wallet') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel" @click="copyShareUrl">{{ t('operations.copyShareLink', 'Copy Share Link') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel disabled:opacity-50" :disabled="!collaborationUrl" @click="copyCollaboratorUrl">{{ t('operations.copyCollaboratorLink', 'Copy Collaborator Link') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel disabled:opacity-50" :disabled="!operatorUrl" @click="copyOperatorUrl">{{ t('operations.copyOperatorLink', 'Copy Operator Link') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel disabled:opacity-50" :disabled="!hasOperatorAccess || isSubmissionPending" @click="rotateCollaboratorLink">{{ t('operations.rotateCollaboratorLink', 'Rotate Collaborator Link') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-sm font-medium text-slate-300 hover:bg-ata-panel disabled:opacity-50" :disabled="!hasOperatorAccess || isSubmissionPending" @click="rotateOperatorLink">{{ t('operations.rotateOperatorLink', 'Rotate Operator Link') }}</button>
                </div>
                <div class="grid gap-4 md:grid-cols-3">
                  <label class="space-y-1 text-sm">
                    <span class="font-medium text-slate-300">Signer ID</span>
                    <input v-model="signerId" class="w-full rounded border border-ata-border px-3 py-1.5 text-sm bg-ata-dark" />
                  </label>
                  <label class="space-y-1 text-sm">
                    <span class="font-medium text-slate-300">Signer Kind</span>
                    <select v-model="signerKind" class="w-full rounded border border-ata-border px-3 py-1.5 text-sm bg-ata-dark">
                      <option value="neo">Neo</option>
                      <option value="evm">EVM</option>
                    </select>
                  </label>
                  <label class="space-y-1 text-sm">
                    <span class="font-medium text-slate-300">Signature Hex</span>
                    <input v-model="signatureHex" class="w-full rounded border border-ata-border px-3 py-1.5 font-mono text-xs bg-ata-dark" />
                  </label>
                </div>
                <div class="mt-4 flex flex-wrap gap-2">
                  <button class="rounded bg-ata-dark px-4 py-2 text-sm font-medium text-white hover:bg-ata-panel disabled:opacity-50" :disabled="!hasSignatureAccess || isSubmissionPending" @click="appendManualSignature">{{ t('operations.appendManualSignature', 'Append Manual Signature') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-4 py-2 text-sm font-medium text-slate-300 hover:bg-ata-panel disabled:opacity-50" :disabled="!hasSignatureAccess || isSubmissionPending" @click="signWithEvmWallet">{{ t('operations.connectEvmWallet', 'Connect EVM Wallet') }}</button>
                </div>
                <p class="mt-4 text-xs text-slate-400">Use manual entry for external multisig collection, or collect an EVM typed-data approval here and keep the relay-ready invocation attached to the draft.</p>
              </section>

              <section class="rounded-lg border border-ata-border bg-ata-panel p-5 shadow-sm">
                <div class="mb-4">
                  <h2 class="text-base font-bold text-white uppercase tracking-widest font-mono">Broadcast & Relay</h2>
                  <p class="text-sm text-slate-400">Choose the final submission path after signatures are attached and relay readiness looks healthy.</p>
                </div>
                <div class="grid gap-3 sm:grid-cols-2">
                  <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Client Broadcast</p>
                    <div class="mt-1 text-sm font-medium text-slate-300">{{ clientBroadcastReady ? 'Invocation Ready' : 'Invocation Missing' }}</div>
                    <p class="mt-1 text-xs text-slate-400">{{ walletConnection.isConnected.value ? 'Neo wallet connected and ready to sign.' : 'Connect a Neo wallet before broadcasting client-side.' }}</p>
                  </div>
                  <div class="rounded-lg border border-ata-border bg-ata-panel p-4">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Relay Submission</p>
                    <div class="mt-1 text-sm font-medium text-slate-300">{{ relayReadiness.label }}</div>
                    <p class="mt-1 text-xs text-slate-400">{{ relayReadiness.detail }}</p>
                  </div>
                </div>
                <div class="mt-4 flex flex-wrap gap-2">
                  <button class="rounded bg-ata-dark px-4 py-2 text-sm font-medium text-white hover:bg-ata-panel disabled:opacity-50" :disabled="!hasOperatorAccess || !clientBroadcastReady || !walletConnection.isConnected.value || isSubmissionPending" :title="getSubmissionButtonLabel('client-broadcast', pendingSubmissionAction)" @click="broadcastWithNeoWallet">{{ pendingSubmissionAction === 'client-broadcast' ? t('sharedDraft.broadcasting', 'Broadcasting…') : t('sharedDraft.broadcastWithNeoWallet', 'Broadcast with Neo Wallet') }}</button>
                  <button class="rounded border border-ata-border bg-ata-panel px-4 py-2 text-sm font-medium text-slate-300 hover:bg-ata-panel disabled:opacity-50" :disabled="!hasOperatorAccess || relayPayloadOptions.length === 0 || isSubmissionPending" :title="getSubmissionButtonLabel('relay-check', pendingSubmissionAction)" @click="checkRelay">{{ pendingSubmissionAction === 'relay-check' ? t('sharedDraft.checkingRelay', 'Checking Relay…') : t('sharedDraft.checkRelay', 'Check Relay') }}</button>
                  <button class="rounded bg-ata-green text-ata-dark px-4 py-2 text-sm font-medium text-white hover:opacity-80 disabled:opacity-50" :disabled="!hasOperatorAccess || !relayReadiness.isReady || isSubmissionPending" :title="getSubmissionButtonLabel('relay-submit', pendingSubmissionAction)" @click="submitViaRelay">{{ pendingSubmissionAction === 'relay-submit' ? t('sharedDraft.submitting', 'Submitting…') : t('sharedDraft.submitViaRelay', 'Submit via Relay') }}</button>
                  <select v-if="relayPayloadOptions.length > 1" v-model="relayPayloadMode" class="rounded border border-ata-border px-3 py-1.5 text-sm bg-ata-dark text-white">
                    <option value="best">Best Available</option>
                    <option value="raw">Signed Raw Tx</option>
                    <option value="meta">Meta Invocation</option>
                  </select>
                </div>
                <p class="mt-4 text-xs text-slate-400">Selected relay payload: <span class="font-semibold text-slate-300">{{ selectedRelayPayloadLabel }}</span></p>
                <div v-if="activeSubmissionReceipt" class="mt-4 rounded-lg border px-4 py-3 text-sm"
                  :class="activeSubmissionReceipt.tone === 'success' ? 'border-ata-green/30 bg-ata-green/10 text-ata-green' : activeSubmissionReceipt.tone === 'error' ? 'border-rose-500/30 bg-rose-500/10 text-rose-400' : 'border-amber-500/30 bg-amber-500/10 text-amber-400'">
                  ...
                  <code v-if="activeSubmissionReceipt.txid" class="mt-2 block break-all rounded border border-white/70 bg-ata-panel/70 px-3 py-1.5 text-xs">{{ activeSubmissionReceipt.txid }}</code>
                  ...
                  <div v-if="submissionReceiptHistoryItems.length > 0" class="mt-4 border-t border-current/10 pt-3">
                    ...
                    <div class="mt-2 space-y-2">
                      <div v-for="item in submissionReceiptHistoryItems" :key="`${item.createdAt}:${item.action}`" class="rounded border border-white/60 bg-ata-panel/60 px-3 py-2">
                ...
                <div v-if="latestBroadcastTxid" class="mt-4 rounded-lg border border-ata-border bg-ata-panel p-4">
                  <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Latest Submission</p>
                  <code class="mt-1 block break-all text-xs text-slate-300">{{ latestBroadcastTxid }}</code>
                  <a v-if="latestBroadcastExplorerUrl" :href="latestBroadcastExplorerUrl" target="_blank" rel="noopener noreferrer" class="mt-2 inline-flex rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-ata-panel">View Latest in Explorer</a>
                </div>
              </section>
            </div>
            ...
            <section class="rounded-lg border border-ata-border bg-ata-panel p-5 shadow-sm">
              <h2 class="text-base font-bold text-white mb-3 uppercase tracking-widest font-mono">{{ t('sharedDraft.recentActivityTitle', 'Recent Activity') }}</h2>
            ...
            <section class="rounded-lg border border-ata-border bg-ata-panel p-5 shadow-sm">
              <h2 class="text-base font-bold text-white mb-3 uppercase tracking-widest font-mono">{{ t('sharedDraft.collectedSignaturesTitle', 'Collected Signatures') }}</h2>
              <div v-if="collectedSignatureCards.length === 0" class="text-sm text-slate-400">{{ t('sharedDraft.noSignaturesYet', 'No signatures have been attached yet.') }}</div>
              <div v-else class="grid gap-3 md:grid-cols-2">
                <div v-for="signature in collectedSignatureCards" :key="signature.key" class="rounded-lg border border-ata-border bg-ata-panel p-4">
                  <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <div class="text-sm font-semibold text-slate-300 break-all">{{ signature.label }}</div>
                      <div class="mt-1 text-xs text-slate-400">Added {{ signature.createdLabel }}</div>
                    </div>
                    <div class="flex flex-wrap justify-end gap-1">
                      <span v-for="badge in signature.badges" :key="badge" class="rounded border border-ata-border bg-ata-panel px-2 py-0.5 text-xs font-medium text-slate-400">{{ badge }}</span>
                    </div>
                  </div>
                  <code class="mt-3 block break-all rounded border border-ata-border bg-ata-panel px-3 py-1.5 text-xs text-slate-400">{{ signature.signaturePreview }}</code>
                  <details class="mt-3">
                    <summary class="cursor-pointer text-xs font-medium text-slate-400 hover:text-white uppercase tracking-widest font-mono">{{ t('sharedDraft.viewFullSignature', 'View Full Signature') }}</summary>
                    <code class="mt-2 block break-all text-xs text-slate-400 bg-ata-panel border border-ata-border p-2 rounded">{{ signature.signatureHex }}</code>
                  </details>
                </div>
              </div>
            </section>
          </div>
          <p v-if="statusMessage" class="mt-6 rounded-lg border border-ata-border bg-ata-panel px-4 py-3 text-sm text-slate-400 font-medium">
            {{ statusMessage }}
          </p>
          <div class="mt-8">
        ...
        <template v-else>
          <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-ata-green/10 mb-6 border border-ata-green/30">
            <svg class="h-8 w-8 text-ata-green animate-fade-in" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <h1 class="text-2xl font-bold text-white tracking-tight mb-2 text-center">Transaction Submitted</h1>
          <p class="text-sm text-slate-400 mb-8 max-w-lg mx-auto text-center">Your transaction has been securely broadcast to the Neo N3 network.</p>
          <div class="bg-ata-panel p-5 rounded-lg border border-ata-border mb-8 inline-block w-full max-w-full">
            <div class="flex items-center justify-between mb-2">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Transaction Hash</p>
              <button @click="copyHash" class="text-xs font-medium text-slate-400 hover:text-white bg-ata-panel border border-ata-border px-2 py-1 rounded">{{ copied ? 'Copied!' : 'Copy' }}</button>
            </div>
            <code class="block text-sm font-mono text-slate-300 break-all bg-ata-panel border border-ata-border p-3 rounded select-all">{{ txid }}</code>
          </div>
