<template>
  <section class="rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur-sm">
    <div class="mb-4 flex items-center justify-between">
      ...
      <button class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 transition-colors" @click="$emit('stage')">{{ t('operations.stageAction', 'Stage') }}</button>
    </div>
    ...
    <div class="mb-5 grid gap-3 lg:grid-cols-4">
      <button
        ...
        :class="option.id === preset ? 'border-neo-500 bg-neo-50 shadow-md ring-2 ring-neo-200' : 'border-slate-200 bg-white hover:border-neo-300 hover:shadow-sm'"
        @click="$emit('update:preset', option.id)"
      >
        <div class="flex items-start gap-2">
          <div class="flex-shrink-0 mt-0.5">
            <svg v-if="option.id === 'invoke'" class="w-5 h-5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg>
            <svg v-else-if="option.id === 'nep17Transfer'" class="w-5 h-5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
            <svg v-else-if="option.id === 'batchCreate'" class="w-5 h-5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
            <svg v-else class="w-5 h-5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-sm font-bold text-slate-900">{{ option.label }}</div>
            <div class="mt-1 text-xs text-slate-500 line-clamp-2">{{ option.description }}</div>
          </div>
        </div>
      </button>
    </div>
    ...
    <div v-if="preset === 'nep17Transfer'" class="space-y-4">
      <div class="grid gap-4 md:grid-cols-2">
        <label class="space-y-1.5 text-sm">
          ...
          <input :value="transferTokenScriptHash" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 font-mono text-xs focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" placeholder="0x..." @input="$emit('update:transferTokenScriptHash', $event.target.value)" />
        </label>
        <label class="space-y-1.5 text-sm">
          <span class="flex items-center gap-1.5 font-medium text-slate-700">
          ...
          <input :value="transferRecipient" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" placeholder="N... or 0x..." @input="$emit('update:transferRecipient', $event.target.value)" />
        </label>
      </div>
      <div class="grid gap-4 md:grid-cols-2">
        <label class="space-y-1.5 text-sm">
          ...
          <input :value="transferAmount" type="number" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" placeholder="0" @input="$emit('update:transferAmount', $event.target.value)" />
        </label>
        <label class="space-y-1.5 text-sm">
          <span class="flex items-center gap-1.5 font-medium text-slate-700">
          ...
          <input :value="transferData" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" placeholder="null or JSON" @input="$emit('update:transferData', $event.target.value)" />
        </label>
      </div>
    </div>

    <div v-else-if="preset === 'batchCreate'" class="space-y-4">
      <div class="rounded-xl bg-neo-50 border border-neo-200 p-4">
        <div class="flex items-start gap-3">
          <svg class="w-5 h-5 text-neo-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <div class="text-sm text-neo-900">
            <p class="font-medium mb-1">Batch Account Creation</p>
            <p class="text-neo-700">Create multiple accounts with shared governance in a single transaction. All accounts will have the same admin and manager configuration.</p>
          </div>
        </div>
      </div>
      ...
      <label class="space-y-1.5 text-sm">
        <span class="flex items-center gap-1.5 font-medium text-slate-700">
          ...
          Account IDs
          <span class="text-xs text-slate-500 font-normal">(JSON array of strings)</span>
        </span>
        <textarea :value="batchAccountIds" rows="3" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 font-mono text-xs focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all resize-none" placeholder='["alice-wallet", "bob-wallet", "charlie-wallet"]' @input="$emit('update:batchAccountIds', $event.target.value)" />
        <p class="text-xs text-slate-500 mt-1">Enter account IDs as a JSON array. Each ID will be created with the same governance settings.</p>
      </label>
      ...
      <div class="border-t border-slate-200 pt-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <label class="space-y-1.5 text-sm md:col-span-2">
            <span class="font-medium text-slate-700">Admin Addresses</span>
            <textarea :value="batchAdmins" rows="2" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 font-mono text-xs focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all resize-none" placeholder='["NXXXaddress1", "0xhash2"]' @input="$emit('update:batchAdmins', $event.target.value)" />
            <p class="text-xs text-slate-500">JSON array of Neo addresses or script hashes</p>
          </label>
          <label class="space-y-1.5 text-sm">
            <span class="font-medium text-slate-700">Admin Threshold</span>
            <input :value="batchAdminThreshold" type="number" min="1" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" @input="$emit('update:batchAdminThreshold', $event.target.value)" />
            <p class="text-xs text-slate-500">Minimum signatures required</p>
          </label>
        </div>
      </div>
      ...
      <div class="border-t border-slate-200 pt-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <label class="space-y-1.5 text-sm md:col-span-2">
            <span class="font-medium text-slate-700">Manager Addresses</span>
            <textarea :value="batchManagers" rows="2" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 font-mono text-xs focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all resize-none" placeholder='[] (optional)' @input="$emit('update:batchManagers', $event.target.value)" />
            <p class="text-xs text-slate-500">Leave empty if no managers needed</p>
          </label>
          <label class="space-y-1.5 text-sm">
            <span class="font-medium text-slate-700">Manager Threshold</span>
            <input :value="batchManagerThreshold" type="number" min="0" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" @input="$emit('update:batchManagerThreshold', $event.target.value)" />
            <p class="text-xs text-slate-500">Set to 0 if no managers</p>
          </label>
        </div>
      </div>
    </div>
    ...
    <div v-else class="grid gap-4 md:grid-cols-3">
      <label v-if="preset === 'multisigDraft'" class="space-y-1.5 text-sm md:col-span-1">
        ...
        <input :value="multisigTitle" class="w-full rounded-xl border border-slate-300 px-3 py-2.5 focus:border-neo-500 focus:ring-2 focus:ring-neo-200 transition-all" @input="$emit('update:multisigTitle', $event.target.value)" />
      </label>
    ...
    <div class="mt-5 rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white px-4 py-3.5 shadow-sm">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">{{ t('operations.presetSummaryLabel', 'Operation Summary') }}</p>
          <p class="text-sm font-medium text-slate-900">{{ summaryTitle }}</p>
          <p class="text-sm text-slate-600 mt-0.5">{{ summaryDetail }}</p>
        </div>
      </div>
    </div>
  </section>
