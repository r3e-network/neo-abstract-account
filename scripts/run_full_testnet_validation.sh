...
usage() {
  cat <<'EOF'
Usage: scripts/run_full_testnet_validation.sh [--skip-local|--skip-live|--smoke-only|--plugin-matrix-only|--paymaster-only]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --skip-live)
      ...
      shift
      ;;
    --smoke-only|--plugin-matrix-only|--paymaster-only)
      live_args+=("$1")
      shift
      ;;
    -h|--help)
