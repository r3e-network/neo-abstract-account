<template>
  <section class="rounded-lg border border-biconomy-border bg-biconomy-panel/80 p-6 shadow-sm backdrop-blur-sm">
    <div class="mb-4 flex items-center justify-between gap-4">
      ...
      <button class="rounded-lg bg-biconomy-orange px-4 py-2 text-sm font-semibold text-white" @click="$emit('append-signature')">{{ t('operations.appendManualSignature', 'Append Manual Signature') }}</button>
    </div>
    <div class="grid gap-4 md:grid-cols-3">
      <label class="space-y-1 text-sm">
        <span class="font-medium text-biconomy-text">{{ t('operations.signerIdLabel', 'Signer ID') }}</span>
        <input :value="signerId" class="w-full rounded-lg border border-biconomy-border px-3 py-2 bg-biconomy-dark text-white" @input="$emit('update:signerId', $event.target.value)" />
      </label>
      <label class="space-y-1 text-sm">
        <span class="font-medium text-biconomy-text">{{ t('operations.signerKindLabel', 'Signer Kind') }}</span>
        <select :value="signerKind" class="w-full rounded-lg border border-biconomy-border px-3 py-2 bg-biconomy-dark text-white" @change="$emit('update:signerKind', $event.target.value)">
      ...
      <label class="space-y-1 text-sm md:col-span-1">
        ...
        <input :value="signatureHex" class="w-full rounded-lg border border-biconomy-border px-3 py-2 font-mono text-xs bg-biconomy-dark" @input="$emit('update:signatureHex', $event.target.value)" />
      </label>
    </div>
    <div class="mt-4 grid gap-2 text-sm text-biconomy-muted md:grid-cols-2">
