<template>
  <section class="glass-panel p-6 shadow-glow-blue">
    <div class="mb-4 flex items-center justify-between gap-3">
      <div>
        ...
        <p class="text-xs text-aa-muted mt-1">
          {{
            t(
              "operations.draftOverviewSubtitle",
              "Overview of staged operation and current status",
            )
          }}
        </p>
      </div>
    ...
    >
      <svg
        aria-hidden="true"
        class="w-10 h-10 text-aa-muted/40 mb-3"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
      ...
      <p class="text-sm font-semibold text-aa-muted">
        {{ t("operations.noDraftItems", "No draft items yet") }}
      </p>
      <p class="text-xs text-aa-muted/60 mt-1">
        {{
          t(
            "operations.noDraftItemsHint",
            "Stage an operation to see its summary here",
          )
        }}
      </p>
    </div>
    <div v-else class="grid gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <div
        ...
        :key="item.label"
        class="rounded-lg border border-aa-border bg-aa-dark/40 p-4 shadow-inner hover:bg-aa-dark/60 transition-colors duration-200"
      >
        <div class="mb-2 flex items-start justify-between gap-2">
          <p class="text-[10px] font-bold uppercase text-aa-orange">
            {{ item.label }}
          </p>
          <button
          ...
          >
            <span class="flex items-center gap-1">
              <svg
                aria-hidden="true"
                v-if="summaryActions[item.label].id === 'open-url'"
                class="w-3 h-3"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
        ...
        <div class="text-sm font-semibold text-aa-text break-all leading-tight">
          {{ item.value }}
        </div>
      </div>
    </div>
...
} from "@/features/operations/draftSummary.js";
...
const displayTitle = computed(() =>
  !props.title ? t("operations.draftOverview", "Draft Overview") : props.title,
);

const summaryItems = computed(() =>
  buildDraftSummaryItems({ draft: props.draft || {}, t }),
);
