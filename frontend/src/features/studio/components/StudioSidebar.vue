  ...
  <aside class="hidden lg:block lg:col-span-4 xl:col-span-3 space-y-6 animate-fade-in" style="animation-delay: 150ms;">
    <div class="bg-slate-800/60 shadow-[0_8px_32px_rgba(0,0,0,0.5)] rounded-2xl border border-slate-700/50 p-6 relative overflow-hidden backdrop-blur-xl">
      ...
      <ul class="space-y-4">
        ...
        <li class="flex items-start gap-3">
          <div :class="computedAddress ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'" class="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors">
            <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
          </div>
          <div>
            <span :class="computedAddress ? 'text-slate-900 font-bold' : 'text-slate-500 font-medium'" class="block text-sm">{{ t('studioPanels.accountAddressReady', 'AA Address Ready') }}</span>
