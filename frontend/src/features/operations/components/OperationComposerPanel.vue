  ...
  <section class="glass-panel p-6">
    ...
    >
      ...
      <p class="text-sm font-medium text-aa-text">
        {{
          t(
            "operations.composerEmptyTitle",
            "Enter a contract hash above to begin composing an operation",
          )
        }}
      </p>
      <p class="mt-1 text-xs text-aa-muted">
        {{
          t(
            "operations.composerEmptyHint",
            "You can search by name, NNS address, or paste a script hash directly.",
          )
        }}
      </p>
    </div>
    <div class="mb-5 grid gap-3 lg:grid-cols-4">
      <button
        ...
        :class="
          option.id === preset
            ? 'border-aa-orange bg-aa-orange/15 shadow-md ring-2 ring-aa-orange'
            : 'border-aa-border bg-aa-panel hover:border-aa-orange hover:shadow-sm'
        "
      ...
      >
        <div class="flex items-start gap-2">
          <div class="flex-shrink-0 mt-0.5">
            ...
            <svg
              aria-hidden="true"
              v-else-if="option.id === 'nep17Transfer'"
              class="w-5 h-5 text-aa-muted"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            ...
            <svg
              aria-hidden="true"
              v-else-if="option.id === 'batchCreate'"
              class="w-5 h-5 text-aa-muted"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="mt-1 text-xs text-aa-muted line-clamp-2">
              {{ option.description }}
            </div>
          </div>
        </div>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <label class="space-y-1.5 text-sm" for="op-composer-token-script-hash">
          ...
          <input
            ...
            :class="{
              'border-aa-error': nep17TransferErrors.transferTokenScriptHash,
            }"
            :placeholder="t('operations.tokenScriptHashPlaceholder', '0x...')"
            @input="onTransferTokenInput"
          />
          <p
            v-if="nep17TransferErrors.transferTokenScriptHash"
            role="alert"
            class="text-xs text-aa-error mt-1"
          >
            {{ nep17TransferErrors.transferTokenScriptHash }}
          </p>
        </label>
        <label class="space-y-1.5 text-sm" for="op-composer-transfer-recipient">
          ...
          <input
            ...
            class="input-field"
            :class="{
              'border-aa-error': nep17TransferErrors.transferRecipient,
            }"
            :placeholder="t('operations.recipientPlaceholder', 'N... or 0x...')"
            @input="onTransferRecipientInput"
          />
          <p
            v-if="nep17TransferErrors.transferRecipient"
            role="alert"
            class="text-xs text-aa-error mt-1"
          >
            {{ nep17TransferErrors.transferRecipient }}
          </p>
        </label>
      </div>
      <div class="grid gap-4 md:grid-cols-2">
        <label class="space-y-1.5 text-sm" for="op-composer-transfer-amount">
          ...
          <input
            ...
            type="number"
            class="input-field"
            :class="{ 'border-aa-error': nep17TransferErrors.transferAmount }"
            placeholder="0"
            @input="onTransferAmountInput"
          />
          <p
            v-if="nep17TransferErrors.transferAmount"
            role="alert"
            class="text-xs text-aa-error mt-1"
          >
            {{ nep17TransferErrors.transferAmount }}
          </p>
        </label>
    ...
    <div v-else-if="preset === 'batchCreate'" class="space-y-4">
      ...
      <label class="space-y-1.5 text-sm" for="op-composer-batch-account-ids">
        ...
        <textarea
          ...
          :class="{ 'border-aa-error': batchCreateErrors.batchAccountIds }"
          :placeholder="
            t(
          ...
          "
          @input="onBatchAccountIdsInput"
        />
        <p
          v-if="batchCreateErrors.batchAccountIds"
          role="alert"
          class="text-xs text-aa-error mt-1"
        >
          {{ batchCreateErrors.batchAccountIds }}
        </p>
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          >
            ...
            <textarea
              ...
              rows="2"
              class="input-field font-mono text-xs resize-none"
              :class="{ 'border-aa-error': batchCreateErrors.batchSigners }"
              :placeholder="
                t(
              ...
              "
              @input="onBatchSignersInput"
            />
            <p
              v-if="batchCreateErrors.batchSigners"
              role="alert"
              class="text-xs text-aa-error mt-1"
            >
              {{ batchCreateErrors.batchSigners }}
            </p>
    ...
    <div
      class="mt-5 rounded-lg border border-aa-border bg-gradient-to-br from-aa-dark to-aa-panel px-4 py-3.5 shadow-sm"
    >
      <div class="flex items-start gap-3">
        <svg
          aria-hidden="true"
          class="w-5 h-5 text-aa-muted flex-shrink-0 mt-0.5"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <div class="flex-1 min-w-0">
          <p class="text-xs font-semibold text-aa-muted uppercase mb-1">
            {{ t("operations.presetSummaryLabel", "Operation Summary") }}
          </p>
