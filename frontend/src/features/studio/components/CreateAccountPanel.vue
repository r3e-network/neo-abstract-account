      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-gradient-to-br from-ata-dark/50 to-ata-panel/50 p-5 rounded-lg border border-ata-border shadow-sm relative overflow-hidden group hover:border-ata-green/50 transition-colors duration-300">
          ...
          <div class="flex justify-between items-center mb-4 relative z-10">
            ...
            <button type="button" class="text-xs text-ata-green hover:text-white font-bold bg-ata-green/20 px-2 py-1 rounded transition-colors" @click="addRow(createForm.signers)">+ Add</button>
          </div>
          <div class="space-y-3 mb-5 relative z-10">
            <div v-for="(signer, index) in createForm.signers" :key="`create-signer-${index}`" class="flex gap-2 group/input">
              <input v-model="createForm.signers[index]" type="text" class="input-field font-mono text-sm py-2 px-3 bg-ata-dark" placeholder="N... or 0x..." />
              <button type="button" class="inline-flex items-center p-2 border border-ata-border rounded-lg bg-ata-panel text-slate-400 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/30 transition-colors" @click="removeRow(createForm.signers, index)">
          ...
          <div class="flex items-center justify-between border-t border-ata-border/60 pt-4 relative z-10">
            ...
            <input v-model.number="createForm.threshold" type="number" min="1" :max="Math.max(1, validCreateSigners.length)" class="input-field w-20 text-center py-1 text-sm font-bold text-slate-300 bg-ata-dark" />
          </div>
        </div>
      </div>
