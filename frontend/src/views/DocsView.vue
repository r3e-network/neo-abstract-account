  ...
  >
    ...
    >
      <div class="flex flex-col md:flex-row gap-8">
        <aside
          :aria-label="t('docs.documentationSidebar', 'Documentation sidebar')"
        ...
        >
          ...
          >
            ...
            <div class="mb-4">
              <div class="relative">
                ...
                <svg
                  aria-hidden="true"
                  v-if="!searchQuery"
                  class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-aa-muted"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <button
                  v-else
                  @click="searchQuery = ''"
                  :aria-label="t('docs.clearSearch', 'Clear search')"
                  class="absolute right-3 top-1/2 -translate-y-1/2 text-aa-muted hover:text-aa-text transition-colors duration-200"
                >
                  <svg
                    aria-hidden="true"
                    class="w-4 h-4"
              ...
              <p
                v-if="searchQuery && filteredDocKeys.length === 0"
                class="mt-2 text-xs text-aa-muted px-2"
              >
                {{ t("docs.noDocsFound", "No docs found") }}
              </p>
            </div>
            <nav class="space-y-1">
              <button
            ...
            <div class="mt-4 pt-4 border-t border-aa-border/60 space-y-1">
              <router-link
                to="/"
                class="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-aa-muted hover:text-aa-text rounded-lg hover:bg-aa-dark transition-all duration-200"
              >
                <svg
                  aria-hidden="true"
                  class="w-3.5 h-3.5"
        ...
        <div class="flex-1 min-w-0">
          ...
          >
            ...
            <transition
              ...
              name="fade-in-up"
              mode="out-in"
              @after-enter="renderMermaidDiagrams"
            >
...
function resolveDocKey(candidate, availableDocs = docs.value) {
  if (
    ...
    Object.prototype.hasOwnProperty.call(availableDocs, candidate)
  ) {
    return candidate;
  }
...
async function renderMermaidDiagrams() {
  ...
  const containers =
    contentRoot.value?.querySelectorAll(".mermaid-container") || [];

  for (const container of containers) {
    try {
      const rawCode = decodeURIComponent(
        container.getAttribute("data-mermaid-code"),
      );
      const { svg } = await mermaid.render(`${container.id}-svg`, rawCode);
      container.innerHTML = svg;
    } catch (err) {
      if (import.meta.env.DEV)
        console.error("[DocsView] Mermaid render failed:", err?.message);
      const pre = document.createElement("pre");
      pre.className = "text-aa-error-light text-xs overflow-auto";
      pre.textContent = t(
        "docs.diagramRenderFailed",
...
const renderMarkdown = async (key) => {
  ...
  try {
    ...
    if (import.meta.env.DEV)
      console.error("[DocsView] Markdown render failed:", err?.message);
...
.markdown-body h1 {
  ...
  background: linear-gradient(
    135deg,
    theme("colors.aa.orange"),
    theme("colors.neo.400")
  );
