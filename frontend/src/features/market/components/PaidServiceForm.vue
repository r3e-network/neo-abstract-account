<template>
  <div class="rounded-lg border border-aa-border bg-aa-dark/50 p-4 space-y-4">
    ...
    <p class="text-xs text-aa-muted leading-6">
      {{
        t(
          "market.vanityPaidDescription",
          "Submit your pattern to our backend service. You pay GAS based on estimated computation time. 1 GAS covers ~2 hours. 80% refunded on timeout.",
        )
      }}
    </p>
    ...
    <div class="rounded-lg border border-aa-border bg-aa-dark/50 p-3 space-y-2">
      <div class="flex justify-between text-sm">
        <span class="text-aa-muted">{{
          t("market.vanityPattern", "Pattern")
        }}</span>
        <span class="font-mono text-aa-text"
          >{{ patternType }}: {{ pattern }}</span
        >
      </div>
      <div class="flex justify-between text-sm">
        <span class="text-aa-muted">{{
      ...
      <div class="flex justify-between text-sm">
        <span class="text-aa-muted">{{
          t("market.vanityTimeout", "Timeout")
        }}</span>
    ...
    <div class="rounded-lg border border-aa-warning/20 bg-aa-warning/5 p-3">
      <p class="text-xs text-aa-warning leading-5">
        {{
          t(
            "market.vanityRefundPolicy",
            "80% of GAS is refunded if the service times out before finding a match. Remaining 20% covers server costs.",
          )
        }}
      </p>
    </div>
