        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            <div v-else class="w-full bg-aa-dark/50 border border-aa-border rounded-lg py-3 px-4 text-sm text-aa-muted text-center">
              {{ t('studioPanels.enterAccountParametersToDerive', 'Enter account parameters below to derive address') }}
            </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div class="space-y-2">
            ...
            <input id="escape-timelock-days" v-model.number="createForm.escapeTimelockDays" type="number" min="7" max="90" class="input-field w-32 text-sm py-2 px-3 bg-aa-dark" placeholder="30" />
            <p class="mt-1 text-xs text-aa-muted">{{ t('studioPanels.escapeTimelockHint', 'Countdown before escape hatch activates (7-90 days)') }}</p>
          </div>
        </div>
        </div>
      </div>
