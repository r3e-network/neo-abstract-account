...
\end{tikzpicture}
\caption{Two-phase verification: mempool uses CheckNativeSignatures, execution uses CheckMixedSignatures for unified N3+EVM authorization.}
...
\end{tikzpicture}
\caption{CheckMixedSignatures algorithm: unified counting of EVM (verifiedSigners) and N3 (CheckWitness) signatures toward threshold.}
...
\end{tikzpicture}
\caption{Transaction type comparison: Pure Neo, Pure EVM, and Mixed N3+EVM authorization modes.}
...
\end{tikzpicture}
\caption{Mixed authorization flow: EVM signs EIP-712 off-chain, Neo embeds signature, both count toward threshold.}
...
\end{tikzpicture}
\caption{Neo N3 transaction layout with AA invocation, verify address, EVM signature, and Neo witnesses.}
...
\end{tikzpicture}
\caption{Threshold verification algorithm: loads configuration, counts Neo witnesses and EVM signatures, accepts if total meets threshold.}
...
\end{tikzpicture}
\caption{System architecture with trust boundaries: off-chain coordination (untrusted) vs on-chain execution (trusted).}
