<template>
  <section class="rounded-lg border border-ata-border bg-ata-panel/80 p-6 shadow-sm backdrop-blur-sm">
    <div class="mb-4">
      ...
      <p class="text-sm text-slate-400 mt-1">Find all accounts where you are an admin or manager</p>
    </div>
    ...
    <div class="space-y-3">
      ...
      <div v-if="error" class="rounded-lg bg-rose-500/10 border border-rose-500/30 p-3 flex items-start gap-2">
        <svg class="w-5 h-5 text-rose-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        <p class="text-sm text-rose-400">{{ error }}</p>
      </div>

      <div v-if="results" class="space-y-3 mt-4">
        <div v-if="results.adminAccounts.length > 0" class="rounded-lg border border-ata-border bg-ata-panel p-4">
          ...
          <div class="space-y-2">
            <button v-for="(account, idx) in results.adminAccounts" :key="idx" class="w-full text-left rounded-lg border border-ata-border bg-ata-panel px-3 py-2.5 hover:border-ata-green hover:bg-ata-green/10 transition-all group" @click="$emit('select', account)">
        ...
        <div v-if="results.managerAccounts.length > 0" class="rounded-lg border border-ata-border bg-ata-panel p-4">
          <h3 class="text-sm font-mono font-semibold text-white uppercase tracking-wider mb-3 flex items-center gap-2 tracking-widest">
          ...
          <div class="space-y-2">
            <button v-for="(account, idx) in results.managerAccounts" :key="idx" class="w-full text-left rounded-lg border border-ata-border bg-ata-panel px-3 py-2.5 hover:border-ata-green hover:bg-ata-green/10 transition-all group" @click="$emit('select', account)">
              <div class="flex items-center justify-between">
                <span class="font-mono text-sm text-white group-hover:text-ata-green">{{ account }}</span>
                <svg class="w-4 h-4 text-slate-400 group-hover:text-ata-green" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
              </div>
        ...
        <div v-if="results.adminAccounts.length === 0 && results.managerAccounts.length === 0" class="rounded-lg border border-ata-border bg-ata-panel p-6 text-center">
          ...
          <p class="text-xs text-slate-400 mt-1">This address is not an admin or manager of any accounts</p>
        </div>
      </div>
    </div>
  </section>
