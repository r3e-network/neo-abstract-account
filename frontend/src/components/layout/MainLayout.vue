<template>
  <div class="min-h-screen bg-ata-dark flex flex-col font-sans text-slate-300 selection:bg-ata-blue selection:text-white transition-colors duration-300 relative overflow-hidden">
    <nav class="sticky top-0 z-50 bg-ata-dark/90 backdrop-blur-md border-b border-ata-border">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 transition-all duration-300 gap-4">
          <div class="flex items-center">
            <div class="flex-shrink-0 flex items-center group">
              <router-link to="/" class="flex items-center gap-3">
                <div class="w-8 h-8 bg-ata-panel border border-ata-border text-white rounded flex items-center justify-center font-bold text-lg shadow-sm transform group-hover:scale-105 transition-transform duration-200 font-mono">
                  N
                </div>
            ...
            <div class="hidden sm:-my-px sm:ml-10 sm:flex sm:space-x-8">
              <router-link to="/" class="border-transparent text-slate-400 hover:text-white hover:border-ata-border inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors duration-200" active-class="border-ata-blue text-white">
                {{ t('nav.home', 'HOME') }}
              </router-link>
              <router-link to="/docs" class="border-transparent text-slate-400 hover:text-white hover:border-ata-border inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors duration-200" active-class="border-ata-blue text-white">
                {{ t('nav.docs', 'DOCS') }}
              </router-link>
            </div>
          </div>
          <div class="flex items-center gap-4">
            <div class="hidden sm:flex items-center gap-1 bg-ata-panel border border-ata-border rounded p-1 text-xs font-semibold text-slate-400">
              <button
                v-for="item in locales"
                :key="item.code"
                @click="setLocale(item.code)"
                :class="locale === item.code ? 'bg-ata-border text-white shadow-sm' : 'text-slate-400 hover:text-slate-300'"
