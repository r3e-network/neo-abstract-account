  ...
  <section class="glass-panel p-6">
      ...
      <div>
        ...
        <p class="text-sm text-aa-muted">
          {{
            t(
              "recovery.subtitle",
              "Recover access to your account using the backup owner escape hatch.",
            )
          }}
        </p>
      </div>
    ...
    <div v-if="currentStep === 0" class="max-w-2xl mx-auto">
      <div class="rounded-xl border border-aa-border bg-aa-panel/60 p-6">
        <div class="flex items-start gap-4 mb-5">
          <div
            class="w-12 h-12 rounded-full bg-neo-500/20 flex items-center justify-center"
          >
            <svg
              aria-hidden="true"
              class="w-6 h-6 text-neo-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-aa-muted mb-4">
              {{
                t(
                  "recovery.step1Desc",
                  "Connect as the backup owner to initiate recovery. Only the configured backup owner can operate the escape hatch.",
                )
              }}
            </p>
          </div>
        ...
        <div class="space-y-4">
          <div>
            <label
              for="backup-owner-address"
              class="block text-sm font-medium text-aa-text mb-2"
              >{{
                t("recovery.backupOwnerAddress", "Backup Owner Address")
              }}</label
            >
            <input
              ...
              :placeholder="
                t('recovery.backupOwnerPlaceholder', 'N... or 0x...')
              "
            ...
            <p
              v-else-if="backupOwnerAddress && !backupOwnerError"
              class="text-xs text-aa-success mt-1"
            >
              <svg
                aria-hidden="true"
          ...
          <div
            class="rounded-lg border border-aa-warning/30 bg-aa-warning/5 p-4"
          >
            <div class="flex items-start gap-3">
              <svg
                aria-hidden="true"
                class="w-5 h-5 text-aa-warning flex-shrink-0 mt-0.5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div>
                <p class="text-sm font-semibold text-aa-warning mb-1">
                  {{ t("recovery.important", "Important") }}
                </p>
                <p class="text-xs text-aa-warning-light">
                  {{
                    t(
                      "recovery.backupOwnerRole",
                      "The backup owner has the power to take control of your account after the escape timelock expires. Only assign this role to addresses you fully trust.",
                    )
                  }}
                </p>
              </div>
          ...
          <div class="flex justify-end">
            ...
            >
              {{
                verifying
                  ? t("recovery.verifying", "Verifying…")
                  : t("recovery.continue", "Continue")
              }}
            </button>
          </div>
        </div>
    ...
    <div v-if="currentStep === 1" class="max-w-2xl mx-auto">
      <div class="rounded-xl border border-aa-border bg-aa-panel/60 p-6">
        <div class="flex items-start gap-4 mb-5">
          <div
            class="w-12 h-12 rounded-full bg-aa-warning/20 flex items-center justify-center"
          >
            <svg
              aria-hidden="true"
              class="w-6 h-6 text-aa-warning"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-aa-muted mb-4">
              {{
                t(
                  "recovery.step2Desc",
                  "Start the escape hatch countdown. You must wait for the timelock period to expire before finalizing recovery.",
                )
              }}
            </p>
          </div>
        ...
        <div class="mb-5 rounded-lg border border-aa-border bg-aa-dark/40 p-4">
          <p class="text-xs font-bold uppercase text-aa-muted mb-3">
            {{ t("recovery.escapeConfig", "Escape Configuration") }}
          </p>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label
                for="escape-timelock"
                class="block text-xs font-medium text-aa-text mb-1"
                >{{
                  t("recovery.timelockDuration", "Timelock Duration")
                }}</label
              >
              <p class="text-sm font-mono text-aa-text">
                {{ escapeTimelock }} {{ t("recovery.seconds", "seconds") }}
              </p>
            </div>
            <div>
              <label
                for="new-verifier"
                class="block text-xs font-medium text-aa-text mb-1"
                >{{ t("recovery.newVerifier", "New Verifier") }}</label
              >
              <p
                class="text-sm font-mono text-aa-text truncate"
                :title="newVerifier"
              >
                {{ formatHash(newVerifier) }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div
          class="rounded-lg border border-aa-warning/30 bg-aa-warning/5 p-4 mb-5"
        >
          <div class="flex items-start gap-3">
            <svg
              aria-hidden="true"
            ...
            <div>
              <p class="text-sm font-semibold text-aa-warning mb-2">
                {{
                  t(
                    "recovery.whatHappensDuringEscape",
              ...
              <ul class="text-xs text-aa-warning-light space-y-1">
                <li class="flex items-start gap-2">
                  ...
                  {{
                    t(
                      "recovery.escapeEffect1",
                      "Current signer access is suspended",
                    )
                  }}
                </li>
                <li class="flex items-start gap-2">
                ...
                <li class="flex items-start gap-2">
                  ...
                  {{
                    t(
                      "recovery.escapeEffect3",
                      "Account is put in recovery mode",
                    )
                  }}
                </li>
              </ul>
        ...
        <div class="flex justify-between">
          ...
          >
            {{
              initiating
                ? t("recovery.initiating", "Initiating…")
                : t("recovery.initiateEscape", "Initiate Escape")
            }}
          </button>
        </div>
      </div>
      ...
      <div class="rounded-xl border-2 border-aa-warning/50 bg-aa-warning/5 p-6">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-aa-muted mb-4">
              {{
                t(
                  "recovery.step3Desc",
                  "The escape hatch must wait for the timelock period to expire before recovery can be finalized.",
                )
              }}
            </p>
          </div>
        ...
        <div class="rounded-lg border border-aa-border bg-aa-panel/50 p-4 mb-5">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-sm font-semibold text-aa-info-light mb-1">
                {{ t("recovery.duringTimelock", "During this time:") }}
              </p>
              <ul class="text-xs text-aa-muted space-y-1">
                <li class="flex items-start gap-2">
                  <svg
                    aria-hidden="true"
                    class="w-4 h-4 flex-shrink-0 mt-0.5 text-aa-success"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                  ...
                  {{
                    t(
                      "recovery.timelockEffect1",
                      "Account state is visible but frozen",
                    )
                  }}
                </li>
                <li class="flex items-start gap-2">
                  ...
                  {{
                    t(
                      "recovery.timelockEffect2",
                      "New operations cannot be staged",
                    )
                  }}
                </li>
                <li class="flex items-start gap-2">
                  ...
                  {{
                    t(
                      "recovery.timelockEffect3",
                      "You can cancel the escape anytime",
                    )
                  }}
                </li>
              </ul>
        ...
        <div class="flex justify-between">
          ...
          >
            {{
              cancelling
                ? t("recovery.canceling", "Canceling…")
                : t("recovery.cancelEscape", "Cancel Escape")
            }}
          </button>
          <button
          ...
          >
            {{
              finalizing
                ? t("recovery.finalizing", "Finalizing…")
                : t("recovery.finalizeRecovery", "Finalize Recovery")
            }}
          </button>
        </div>
      </div>
    ...
    <div v-if="currentStep === 3" class="max-w-2xl mx-auto">
      <div class="rounded-xl border-2 border-aa-success/30 bg-aa-success/5 p-6">
        <div class="flex items-start gap-4 mb-5">
          <div
            class="w-12 h-12 rounded-full bg-aa-success/20 flex items-center justify-center"
          >
            <svg
              aria-hidden="true"
              class="w-6 h-6 text-aa-success"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-aa-muted mb-4">
              {{
                t(
                  "recovery.step4Desc",
                  "Timelock has expired. Finalize the recovery to take full control of the account.",
                )
              }}
            </p>
          </div>
        ...
        <div
          class="mb-5 rounded-lg border border-aa-success/30 bg-aa-success/10 p-4"
        >
          <div class="flex items-start gap-3">
            <svg
              aria-hidden="true"
              class="w-5 h-5 text-aa-success flex-shrink-0 mt-0.5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <div>
              <p class="text-sm font-semibold text-aa-success mb-1">
                {{ t("recovery.finalizeSummary", "Recovery will:") }}
              </p>
              <ul class="text-xs text-aa-success-light space-y-1">
                <li class="flex items-start gap-2">
                  ...
                  {{
                    t(
                      "recovery.finalizeEffect1",
                      "Replace current verifier with backup owner",
                    )
                  }}
                </li>
                <li class="flex items-start gap-2">
                  ...
                  {{
                    t(
                      "recovery.finalizeEffect2",
                      "Unfreeze the account for normal operation",
                    )
                  }}
                </li>
                <li class="flex items-start gap-2">
        ...
        <div class="flex justify-between">
          ...
          >
            {{
              finalizing
                ? t("recovery.finalizing", "Finalizing…")
                : t("recovery.confirmFinalize", "Confirm Finalize")
            }}
          </button>
        </div>
      </div>
    ...
    <div v-if="currentStep === 4" class="max-w-2xl mx-auto">
      <div
        class="rounded-xl border-2 border-aa-success/50 bg-aa-success/10 p-8 text-center"
      >
        <div
          class="mx-auto w-16 h-16 rounded-full bg-aa-success/20 flex items-center justify-center mb-4"
        >
          <svg
            aria-hidden="true"
            class="w-8 h-8 text-aa-success"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <p class="text-sm text-aa-muted mb-6">
          {{
            t(
              "recovery.completeDesc",
              "Your account has been successfully recovered. You now have full control as the backup owner.",
            )
          }}
        </p>
        <div class="space-y-3">
          ...
          <router-link
            :to="{ path: '/docs', query: { doc: 'recoveryGuide' } }"
...
function validateBackupOwner() {
  ...
  if (addr.startsWith("N") && addr.length !== 34) {
    backupOwnerError.value = t(
      "recovery.invalidNeoAddress",
      "Invalid Neo address",
    );
    return;
  }
