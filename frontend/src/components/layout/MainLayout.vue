      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 transition-all duration-200 gap-4">
          ...
          <div class="flex items-center gap-4">
            ...
            <div v-if="didConnected" class="flex items-center gap-3 bg-aa-panel border border-aa-border rounded px-3 py-1.5 shadow-sm">
            ...
            <div v-if="isConnected" class="flex items-center gap-3 bg-aa-panel border border-aa-border rounded px-3 py-1.5 shadow-sm">
            ...
            <div v-else class="animate-fade-in flex items-center gap-2">
              <button v-if="didAvailable && !didConnected" @click="connectDid" :class="{ 'btn-loading': isDidConnecting }" :disabled="isDidConnecting" class="btn-secondary btn-xs font-bold">{{ isDidConnecting ? t('nav.connecting', 'Connecting…') : t('nav.connectDid', 'Connect Web3Auth') }}</button>
        ...
        <div class="scroll-fade-container relative">
          ...
          <div v-if="didConnected" class="flex items-center gap-1.5 shrink-0">
          ...
          <div v-if="isConnected" class="flex items-center gap-1.5 shrink-0">
          ...
          <div v-else class="flex items-center gap-1.5 shrink-0">
            <button v-if="didAvailable && !didConnected" @click="connectDid" :class="{ 'btn-loading': isDidConnecting }" :disabled="isDidConnecting" class="btn-secondary btn-xs font-bold">{{ isDidConnecting ? t('nav.connecting', '…') : t('nav.connectDid', 'DID') }}</button>
...
const { isConnected, truncatedAddress, connect, disconnect, isConnecting: isWalletConnecting } = useWalletConnection();
const { isConfigured: didAvailable, isConnected: didConnected, shortDid: didShort, connectDid, disconnectDid, isConnecting: isDidConnecting } = useDidConnection();
